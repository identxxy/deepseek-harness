 ...
 */
export function KittyBrowser({ t, wide, activePaneId, expandSidebar, useCatalog, useStore, actions, load, refresh, open, backHome }: KittyBrowserProps) {
  const { value, loading, failed } = useCatalog(s => s);
  const { history, wideScreen } = useStore(s => s);
  const view = useStore(s => activePaneId === null ? undefined : s.panes[activePaneId]);
  const { selected = '', result = null, follow = true } = view ?? {};
  ...
  async function createWindow() {
    if (!pane || !view || activePaneId === null || busy.current) return;
    busy.current = true;
    setCreating(true);
    setNotice('');
    try {
      ...
      if (alive.current) {
        actions.created(activePaneId, created.id);
        void refresh();
      }
    } catch (error) {
      if (alive.current) {
        if (error instanceof Error && error.message === 'stale_target') {
          actions.stale(activePaneId, view.inputVersion);
          void refresh();
  ...
  return <section className="dsh-kitty-browser" aria-label={t('windows')}>
    ...
    {options && <div className="dsh-kitty-browser-options">
      <div className="dsh-kitty-toolbar">
        <Button aria-pressed={follow} disabled={!view} onClick={() => { if (activePaneId !== null) actions.setFollow(activePaneId, !follow); }}>{t('follow')}</Button>
