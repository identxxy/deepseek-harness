...
**Append newline**, beside **Send**, defaults to checked: Send and Ctrl/⌘ Enter paste the draft, then send Enter. Uncheck it to omit that final Enter; line breaks already in the draft remain unchanged. Each split pane retains its own choice across window selections until reload. **Paste only** always omits Enter. Open **Terminal keys** (#) for the **↑ ↓ ← →** buttons, which send individual direction keys without Enter and preserve the draft. **Alt + ↑** sends that shortcut to the selected terminal, including focusing pending Codex questions. The shortcut appears first on phones and preserves the draft. Choosing an image creates a removable preview; sending combines the image reference and caption in one paste. One image can be staged at a time. Clipboard image paste and file drop use the same flow.
...
The Client occupies the keyed `sidebar.page`, `workspace.panel.header` and `workspace.panel.composer` entries and the `workspace.panel` body in the native layout. The layout addresses the single composer to its focused pane. The plugin stores terminal selection, draft, staged image, send status and follow state by layout pane ID; line-width and scrollback preferences and the authenticated catalog are shared. Input requests retain their original pane and selection version, so late receipts cannot clear a replacement draft. Pane selections remain transient across layout remounts and are never persisted. The shell owns responsive navigation and browser history. The terminal shares conversation/composer styles, buttons, icons and attachment presentation with DSH. Screen content is an ANSI terminal snapshot, not a structured DSH transcript.
