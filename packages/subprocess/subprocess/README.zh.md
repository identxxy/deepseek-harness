...
- `spawnTerminal(spec)` 是唯一的非管道原语。其必填终端类型同时配置 PTY 与子进程的 `TERM`。其句柄负责真实 PTY、UTF-8 文本 I/O、活动期间的尺寸调整、前台进程组检查／信号发送，以及一项须等待的 `terminate()` 操作；该操作会使提供方仍可观察到的每个会话成员完全停稳，并结算包括尺寸调整在内的在途句柄调用。进程退出后，输入与尺寸调整都会拒绝；提供方会记录执行基底特有的可观察性限制。spec 信号只取消分配；句柄一经发布，便负责自身生命周期。顶层进程退出时，输出流在已排队输出之后结束；仍处于活动状态的传输若发生故障，会使 `done` 拒绝。这些操作保留为一项执行基底原语，因为普通管道无法分配或调整控制终端，也无法清理终端会话成员；就绪状态、scrollback 和所有者策略仍归 PTY 消费方所有。
