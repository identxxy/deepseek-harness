...
Syntax highlighting uses one synchronous Shiki singleton, constructed by the first supported highlighting request rather than module import. `CodeBlock` and `ReadBlock` defer that request until viewport activation; pages without activated supported code, including Kitty-only views, do not construct the singleton. TypeScript, shell and JSON remain bundled and are pre-tokenized on construction outside the user-content scan budget; 23 other grammars remain dynamically imported. The first activated supported block pays the initialization cost ([initialization decision](../../../.agents/notes/implemented/architecture/2026-09-11-demand-initialized-syntax-highlighter.md), [viewport activation](../../../.agents/notes/implemented/architecture/2026-08-31-viewport-activated-syntax-highlighting.md)).
