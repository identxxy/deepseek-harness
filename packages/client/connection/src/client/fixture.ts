...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const rpc: ClientConnectionRpc = {
    call(channel, endpoint, payload, signal = new AbortController().signal) {
      if (channel !== '/api') {
      ...
      switch (endpoint) {
        ...
        case 'consoles/read': return consoleRemotes.read(args.request as Parameters<typeof consoleRemotes.read>[0], signal)
