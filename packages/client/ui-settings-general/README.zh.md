...
低于 640px 时，设置面板占满视口并使用由 History 支持的两级导航：打开时显示完整分类列表，选择分类后其已挂载内容占满宽度，浏览器 Back 返回列表，Forward 恢复该分类。Settings 条目保留其他 History 字段，包括移动端会话目的地；关闭只退出当前面板拥有的条目。更宽的视口继续同时显示导航栏与内容栏。
## 目录
...
引导账本按升序投影，每次只挂载一个步骤；已挂载但仍在判定私有事实的步骤渲染 null，因此判定期间不绘制也不阻塞任何内容。注册方持有持久化完成状态、能力就绪状态、文案、变更操作与可见包装，因此独立注册的流程无法堆叠，外壳也不会成为第二个配置事实来源。可见步骤自行持有弹窗框架与应用根节点 `inert` 生命周期；当前注册方会收到该条目的 id、`complete()` 和 `openSection(id)` 回调，完成或跳过当前步骤后，所有权转交给下一项。
