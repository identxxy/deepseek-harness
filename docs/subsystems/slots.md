...
Plugin panes render their body through `workspace.panel` and can register an actor-ID-keyed `workspace.panel.header` entry for the title and navigation. Layout retains split and close controls and provides the default title when a header entry is absent. A plugin body owns its input area within `workspace.panel`. Explicit Agent panes pass `compactInput: true` and `active` through `ConvOwnerProps`; the conversation shell forwards them as `compact` and `active` to the default `conversation.composer.bar`. Composer takeovers retain their own owner props.
