...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const clientDist = join(REPO_ROOT, 'apps/web/dist')
  const clientDistBackup = join(world, 'client-dist')
  await cp(clientDist, clientDistBackup, { recursive: true })
  ...
  try {
    ...
    await rm(clientDist, { recursive: true, force: true }).catch((error: unknown) => failures.push(error))
    await cp(clientDistBackup, clientDist, { recursive: true }).catch((error: unknown) => failures.push(error))
